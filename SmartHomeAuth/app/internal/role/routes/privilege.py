...
@router.post("")
async def add(data: PrivilegeForm, user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
async def get(user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
@router.delete("/{id}")
async def get(id:str, user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
