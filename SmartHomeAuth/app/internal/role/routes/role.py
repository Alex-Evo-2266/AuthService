...
from app.internal.auth.depends.auth import user_preveleg_oauth_dep
...
@router.post("")
async def add(data: RoleForm, user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
		await add_role(data)
		return "ok"
...
async def get_role(user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
@router.delete("/{id}")
async def delete_role(id:str, user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
async def edit_privilege(data:EditPrivilegeRoleForm, user_data:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
