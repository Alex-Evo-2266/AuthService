...
@router.post("")
async def add(data: UserForm, session:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
		await add_user(data)
		return "ok"
...
async def get(session:SessionDepData = Depends(user_oauth_dep)):
	try:
...
@router.put("")
async def edit(data:UserEditSchema, session:SessionDepData = Depends(user_oauth_dep)):
	try:
...
@router.put("/{id}")
async def edit(id:str, data:UserEditSchema, session:SessionDepData = Depends(user_oauth_dep)):
	try:
		user = await get_user(id)
...
@router.delete("/{id}")
async def delete(id: str, session:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
@router.patch("/role")
async def level(data: UserEditLevelSchema, session:SessionDepData = Depends(user_preveleg_oauth_dep(BASE_ROLE.ADMIN))):
	try:
...
async def edit_password(data: UserEditPasswordSchema, session:SessionDepData = Depends(user_oauth_dep)):
	try:
